...
catalog_backup=""
if [[ -f "$APP_ROOT/web/data/airband-channels.json" ]]; then catalog_backup="$(mktemp)"; sudo cp "$APP_ROOT/web/data/airband-channels.json" "$catalog_backup"; fi
sudo cp -a "$(dirname "$0")/.."/. "$APP_ROOT"/
if [[ -n "$catalog_backup" ]]; then sudo cp "$catalog_backup" "$APP_ROOT/web/data/airband-channels.json"; rm -f "$catalog_backup"; fi
