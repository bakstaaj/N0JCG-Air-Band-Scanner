...
async function startPcmAudio() {
  stopPcmAudio(); audioAbort = new AbortController(); audioContext = new (window.AudioContext || window.webkitAudioContext)({sampleRate: 24000}); await audioContext.resume(); const workletAvailable = audioContext.audioWorklet && typeof audioContext.audioWorklet.addModule === "function"; if (workletAvailable) { await audioContext.audioWorklet.addModule(`/audio-worklet.js?v=pcm-player-v1`); audioNode = new AudioWorkletNode(audioContext, "n0jcg-pcm-player"); audioNode.connect(audioContext.destination); } else { audioNextTime = audioContext.currentTime + 1.0; } $("audioStatus").hidden = false; $("audioStatus").textContent = workletAvailable ? "Live AM audio buffered - use system/browser volume." : "Live AM audio connected using browser compatibility mode - use system/browser volume.";
  ...
  try { while (true) { const part = await reader.read(); if (part.done) break; const bytes = new Uint8Array(carry.length + part.value.length); bytes.set(carry); bytes.set(part.value, carry.length); const usable = bytes.length - (bytes.length % 2); if (!usable) { carry = bytes; continue; } const samples = new Float32Array(usable / 2); const view = new DataView(bytes.buffer, bytes.byteOffset, usable); for (let i = 0; i < samples.length; i++) samples[i] = view.getInt16(i * 2, true) / 32768; carry = bytes.slice(usable); if (audioNode) audioNode.port.postMessage(samples.buffer, [samples.buffer]); else queueAudioBuffer(samples); } if (!audioNode && audioPendingSamples) { const merged = new Float32Array(audioPendingSamples); let offset = 0; audioPending.forEach((part) => { merged.set(part, offset); offset += part.length; }); audioPending = []; audioPendingSamples = 0; scheduleAudioBuffer(merged); } } catch (error) { if (error.name !== "AbortError") throw error; }
}
