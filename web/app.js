...
async function ensurePcmPlayer() { if (!audioContext) { audioContext = new (window.AudioContext || window.webkitAudioContext)({latencyHint: "interactive"}); const gain = audioContext.createGain(); gain.connect(audioContext.destination); if (audioContext.audioWorklet && window.AudioWorkletNode) { await audioContext.audioWorklet.addModule(`/airband-pcm-worklet.js?v=3`); audioNode = new AudioWorkletNode(audioContext, "airband-pcm-player", {numberOfInputs: 0, numberOfOutputs: 1, outputChannelCount: [1], processorOptions: {inputRate: 24000, startSamples: 48000, maxSamples: 96000}}); audioNode.connect(gain); } else if (window.AirbandPcmRing) { audioRing = new window.AirbandPcmRing(audioContext, gain); } else { throw new Error("PCM browser audio is not ready"); } } await audioContext.resume(); }
async function startPcmAudio() { await ensurePcmPlayer(); if (audioAbort) audioAbort.abort(); audioAbort = new AbortController(); audioPending = new Uint8Array(0); if (audioNode) audioNode.port.postMessage({type: "reset"}); if (audioRing) audioRing.reset(); $("audioStatus").hidden = false; $("audioStatus").textContent = "Live PCM audio connected - use system/browser volume."; const response = await fetch(`/api/audio.pcm?listen=${Date.now()}`, {cache: "no-store", signal: audioAbort.signal}); if (!response.ok || !response.body) throw new Error("Live PCM stream unavailable"); audioReader = response.body.getReader(); try { while (true) { const result = await audioReader.read(); if (result.done) break; const bytes = new Uint8Array(audioPending.length + result.value.length); bytes.set(audioPending); bytes.set(result.value, audioPending.length); const usable = bytes.length - (bytes.length % 2); audioPending = bytes.slice(usable); if (!usable) continue; const samples = new Int16Array(bytes.buffer, bytes.byteOffset, usable / 2); const copy = new Int16Array(samples); if (audioNode) audioNode.port.postMessage({type: "pcm", samples: copy.buffer}, [copy.buffer]); else audioRing.enqueue(copy); } } catch (error) { if (error.name !== "AbortError") throw error; } }
