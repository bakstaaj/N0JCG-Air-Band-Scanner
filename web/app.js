...
async function ensurePcmPlayer() { if (!audioContext) { audioContext = new (window.AudioContext || window.webkitAudioContext)({latencyHint: "interactive"}); const gain = audioContext.createGain(); gain.connect(audioContext.destination); if (window.AirbandPcmRing) { audioRing = new window.AirbandPcmRing(audioContext, gain); } else { throw new Error("PCM browser audio is not ready"); } } await audioContext.resume(); }
