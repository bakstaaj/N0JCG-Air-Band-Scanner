...
async function ensurePcmPlayer() { if (!audioContext) { audioContext = new (window.AudioContext || window.webkitAudioContext)({latencyHint: "interactive"}); const gain = audioContext.createGain(); gain.connect(audioContext.destination); if (audioContext.audioWorklet && window.AudioWorkletNode) { await audioContext.audioWorklet.addModule(`/airband-pcm-worklet.js?v=2`); audioNode = new AudioWorkletNode(audioContext, "airband-pcm-player", {numberOfInputs: 0, numberOfOutputs: 1, outputChannelCount: [1], processorOptions: {inputRate: 24000, startSamples: 48000, maxSamples: 96000}}); audioNode.connect(gain); } else if (window.AirbandPcmRing) { audioRing = new window.AirbandPcmRing(audioContext, gain); } else { throw new Error("PCM browser audio is not ready"); } } await audioContext.resume(); }
