...
function stopPcmAudio() { if (audioAbort) audioAbort.abort(); audioAbort = null; if (audioWavTimer) clearTimeout(audioWavTimer); audioWavTimer = null; audioNextPlayTime = 0; if (audioGainNode) audioGainNode.disconnect(); audioGainNode = null; if (audioContext) audioContext.close(); audioContext = null; }

async function ensurePcmPlayer() { if (!audioContext) { audioContext = new (window.AudioContext || window.webkitAudioContext)({latencyHint: "interactive"}); audioGainNode = audioContext.createGain(); audioGainNode.gain.value = 0; audioGainNode.connect(audioContext.destination); } await audioContext.resume(); }
async function pumpWavAudio() { if (!audioContext || !audioAbort || audioAbort.signal.aborted) return; try { const response = await fetch(`/api/audio.wav?chunk=${Date.now()}`, {cache: "no-store", signal: audioAbort.signal}); if (!response.ok) throw new Error("Live WAV audio unavailable"); const data = await response.arrayBuffer(); const decoded = await audioContext.decodeAudioData(data.slice(0)); const source = audioContext.createBufferSource(); source.buffer = decoded; source.connect(audioGainNode); const startAt = Math.max(audioContext.currentTime + 0.04, audioNextPlayTime); source.start(startAt); audioNextPlayTime = startAt + decoded.duration; const buffered = audioNextPlayTime - audioContext.currentTime; audioWavTimer = setTimeout(() => pumpWavAudio(), buffered > 1.4 ? 180 : 20); } catch (error) { if (error.name !== "AbortError") $("audioStatus").textContent = error.message; } }
async function startPcmAudio() { await ensurePcmPlayer(); if (audioAbort) audioAbort.abort(); audioAbort = new AbortController(); audioNextPlayTime = audioContext.currentTime; $("audioStatus").hidden = false; $("audioStatus").textContent = "Live WAV audio connected - use system/browser volume."; await pumpWavAudio(); }

async function run(action) { try { await action(); } catch (error) { $("audioStatus").hidden = false; $("audioStatus").textContent = error.message; } }
async function scan(nearby = scanScope === "nearby") { await ensurePcmPlayer(); $("audioStatus").hidden = false; $("audioStatus").textContent = nearby ? "Scanning known FAA channels within the saved radius..." : "Scanning 118.000-136.975 MHz..."; const state = await api(nearby ? "/api/scan/nearby" : "/api/scan", {method: "POST", body: "{}"}); render(state); if (!state.running || !state.tuned) throw new Error(state.error || "No Airband candidate passed the SNR threshold."); await startPcmAudio(); }
...
function updateAudioDiagnostics() { if (!audioContext) return; window.__airbandAudioDiagnostics = {playback: "scheduled-wav", queued_seconds: Math.max(0, audioNextPlayTime - audioContext.currentTime)}; }
