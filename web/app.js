...
async function activateLicense() { const licenseSerial = String($("licenseSerialInput").value || "").trim(); const email = String($("licenseEmailInput").value || "").trim(); if (!licenseSerial || !email) { $("registrationStatusText").textContent = "Enter the license S/N and registered email address."; return; } $("activateLicenseBtn").disabled = true; $("registrationStatusText").textContent = "Contacting N0JCG licensing service…"; try { const result = await api("/api/license/activate", {method: "POST", body: JSON.stringify({license_serial: licenseSerial, email})}); $("licenseSerialInput").value = ""; render(result); $("registrationStatusText").textContent = "License activated for this installation."; } catch (error) { $("registrationStatusText").textContent = `Activation failed: ${error.message}`; $("activateLicenseBtn").disabled = false; } }
async function tune(frequencyHz) { stopPcmAudio(); const state = await api("/api/select", {method: "POST", body: JSON.stringify({frequency_hz: frequencyHz})}); render(state); await startPcmAudio(); if (audioGainNode && audioContext) { const open = Boolean(state.settings?.tuning?.squelch_open); audioGainNode.gain.setTargetAtTime(open ? 1 : 0, audioContext.currentTime, 0.02); } }
async function findAirport() { const data = await api(`/api/airport?code=${encodeURIComponent($("airport").value)}`); $("airportResults").innerHTML = data.channels.length ? data.channels.map((c) => `<div class="result"><span><strong>${esc(c.frequency_use)}</strong><small>${fmt(c.frequency_hz)}</small></span><button type="button" data-freq="${c.frequency_hz}">Tune</button></div>`).join("") : "No channels found."; document.querySelectorAll("[data-freq]").forEach((button) => button.addEventListener("click", () => run(() => tune(Number(button.dataset.freq))))); }
