...
function channelControls(channel) {
  ...
  const control = channel.scan_control || ((window.channelControls || {})[String(frequency)] || {mode: "active"});
...
function renderSettings(settings) {
  ...
  window.channelControls = settings.channel_controls || {};
...
async function findAirport() { const data = await api(`/api/airport?code=${encodeURIComponent($("airport").value)}`); $("airportResults").innerHTML = data.channels.length ? data.channels.map((c) => `<div class="result"><span><strong>${esc(c.frequency_use)}</strong><small>${fmt(c.frequency_hz)}</small></span><span class="channel-row-actions"><button type="button" data-freq="${c.frequency_hz}">Tune</button>${channelControls(c)}</span></div>`).join("") : "No channels found."; document.querySelectorAll("[data-freq]").forEach((button) => button.addEventListener("click", () => run(() => tune(Number(button.dataset.freq))))); bindChannelControls(); }
async function loadNearby() { const data = await api("/api/nearby"); window.channelControls = data.channel_controls || {}; $("nearbySummary").textContent = `${data.channels.length} channels within ${Number(data.radius_miles).toFixed(1)} miles of ${data.location.label}.`; $("nearbyChannels").innerHTML = data.channels.length ? data.channels.map((c) => `<div class="nearby-row"><span><strong>${esc(c.serviced_facility)}</strong> ${esc(c.frequency_use)}<small>${fmt(c.frequency_hz)} · ${esc(c.serviced_facility_name || "")} · ${c.distance_miles} mi</small></span><span class="channel-row-actions"><button type="button" data-nearby-freq="${c.frequency_hz}">Tune</button>${channelControls(c)}</span></div>`).join("") : "No FAA channels found in this radius."; document.querySelectorAll("[data-nearby-freq]").forEach((button) => button.addEventListener("click", () => run(() => tune(Number(button.dataset.nearbyFreq))))); bindChannelControls(); }
