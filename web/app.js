...
async function startPcmAudio() { await ensurePcmPlayer(); if (audioAbort) audioAbort.abort(); audioAbort = new AbortController(); audioNextPlayTime = audioContext.currentTime; $("audioStatus").hidden = false; $("audioStatus").textContent = "Live chunked WAV audio connected - use system/browser volume."; try { while (!audioAbort.signal.aborted) { const response = await fetch(`/api/audio.chunk.wav?listen=${Date.now()}`, {cache: "no-store", signal: audioAbort.signal}); if (!response.ok) { if (response.status === 503) { await new Promise((resolve) => setTimeout(resolve, 200)); continue; } throw new Error("Live audio chunk unavailable"); } const data = await response.arrayBuffer(); const decoded = await audioContext.decodeAudioData(data.slice(0)); const source = audioContext.createBufferSource(); source.buffer = decoded; source.connect(audioGainNode); const startAt = Math.max(audioContext.currentTime + 0.04, audioNextPlayTime); source.start(startAt); audioNextPlayTime = startAt + decoded.duration; } } catch (error) { if (error.name !== "AbortError") throw error; } }
