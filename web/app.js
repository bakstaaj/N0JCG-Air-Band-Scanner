...
function stopPcmAudio() { if (audioAbort) audioAbort.abort(); audioAbort = null; audioSources.forEach((source) => { try { source.stop(); } catch (_) {} }); audioSources = []; if (audioNode) audioNode.disconnect(); audioNode = null; if (audioContext) audioContext.close(); audioContext = null; audioNextTime = 0; }
...
async function startPcmAudio() {
  ...
  try { while (true) { const part = await reader.read(); if (part.done) break; const bytes = new Uint8Array(carry.length + part.value.length); bytes.set(carry); bytes.set(part.value, carry.length); const usable = bytes.length - (bytes.length % 2); if (!usable) { carry = bytes; continue; } const samples = new Float32Array(usable / 2); const view = new DataView(bytes.buffer, bytes.byteOffset, usable); for (let i = 0; i < samples.length; i++) samples[i] = view.getInt16(i * 2, true) / 32768; carry = bytes.slice(usable); const buffer = audioContext.createBuffer(1, samples.length, 24000); buffer.copyToChannel(samples, 0); const source = audioContext.createBufferSource(); source.buffer = buffer; source.connect(audioContext.destination); const startAt = Math.max(audioNextTime, audioContext.currentTime + 0.1); source.start(startAt); audioNextTime = startAt + buffer.duration; audioSources.push(source); source.onended = () => { audioSources = audioSources.filter((item) => item !== source); }; } } catch (error) { if (error.name !== "AbortError") throw error; }
}
