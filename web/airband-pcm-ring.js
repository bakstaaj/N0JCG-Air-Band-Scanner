(function () {
  ...
  class AirbandPcmRing {
    ...
    enqueue(samples) { for (const sample of samples) { if (this.count >= 96000) { this.readIndex = (this.readIndex + 1) % this.buffer.length; this.count -= 1; this.droppedSamples += 1; } this.buffer[this.writeIndex] = sample; this.writeIndex = (this.writeIndex + 1) % this.buffer.length; this.count += 1; } }
    render(outputBuffer) { const channel = outputBuffer.getChannelData(0); channel.fill(0); if (!this.started) { if (this.count < 48000) return; this.started = true; } const queueError = (this.count - 48000) / 96000; this.rateCorrection = Math.max(-0.01, Math.min(0.01, queueError * 0.02)); const step = (24000 / this.context.sampleRate) * (1 + this.rateCorrection); for (let i = 0; i < channel.length; i += 1) { if (this.count < 2) { this.started = false; this.underruns += 1; break; } const first = this.buffer[this.readIndex]; const second = this.buffer[(this.readIndex + 1) % this.buffer.length]; channel[i] = (first + ((second - first) * this.phase)) / 32768; this.phase += step; while (this.phase >= 1 && this.count > 1) { this.phase -= 1; this.readIndex = (this.readIndex + 1) % this.buffer.length; this.count -= 1; } } }
    ...
    diagnostics() { return {queued_samples: this.count, queued_seconds: this.count / 24000, underruns: this.underruns, dropped_samples: this.droppedSamples, rate_correction: this.rateCorrection}; }
  }
